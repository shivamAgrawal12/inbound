...
class CustomAuthManager {
  ...
  Future signOut() async {
    ...
    inboundV3GSGlobalAuthUserSubject.add(
      InboundV3GSGlobalAuthUser(loggedIn: false),
    );
    persistAuthData();
  }

  Future<InboundV3GSGlobalAuthUser?> signIn({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
  ...
  InboundV3GSGlobalAuthUser? _updateCurrentUser({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
    ...
    final updatedUser = InboundV3GSGlobalAuthUser(
      loggedIn: true,
      uid: authUid,
      userData: userData,
    );
    inboundV3GSGlobalAuthUserSubject.add(updatedUser);
    persistAuthData();
    return updatedUser;
  }
...
InboundV3GSGlobalAuthUser? currentUser;
